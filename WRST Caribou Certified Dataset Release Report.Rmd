...
## GPS Collar Deployment History

```{r, echo=FALSE}

Sql = "SELECT   Herd, Year, [Collars deployed], Mortalities, Retrieved, Disposed
FROM            Summary_CollarsDeployedByYear
ORDER BY ProjectID, Year"
DF = GetDataFrame(Sql)

```

```{r, echo=FALSE}
# Deployments history by herd
ggplot(DF) +
  geom_col(aes(x=Year,y=`Collars deployed`,na.rm=TRUE)) +
  theme_minimal() + 
  facet_grid(rows=vars(Herd))
  theme_minimal()  +
...
```{r, echo=FALSE, warning=FALSE,fig.cap=paste("Figure ",sep="")}
Sql = "SELECT        Herd, Year, COUNT(DISTINCT SightingDate) AS [Survey days]
FROM            Dataset_Radiotracking
GROUP BY Herd, Year
ORDER BY Herd, Year"
DF = GetDataFrame(Sql)
#head(DF)

ggplot(DF,na.rm=TRUE) +
  geom_col(aes(x=Year,y=`Survey days`),na.rm=TRUE) +
  geom_text(aes(x=Year,y=`Survey days` + 5,label=`Survey days`)) +
  theme_minimal() + 
  facet_grid(rows=vars(Herd)) +
  scale_x_continuous(breaks = seq(min(DF$Year), max(DF$Year), by = 1)) +
  theme_minimal()  +
  ggtitle("Radiotracking survey days")
  theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5)) 
