...
SurveyName = "2022-10-14 Chisana Caribou Composition Survey"


```

---
title: "Dataset Release Report: Abundance and Composition of the Chisana and Mentasta Caribou Herds in Wrangell - St. Elias National Park and Preserve, Alaska"
author: ""
date: "`r print(format(Sys.Date(), "%B %d, %Y"))`"
output:
  html_document:
  toc: yes
---
...
# Park boundaries for the map below are in spatial data files in geojson format are at https://irma.nps.gov/DataStore/Reference/Profile/2303652
...
Table `r TableCount`. Animal counts by sex, size.
...
#Proportion of bulls composition bar chart
SmallBull = CompDF$`Small bull` / CompDF$Bull
MediumBull = CompDF$`Medium bull` / CompDF$Bull
LargeBull = CompDF$`Large bull` / CompDF$Bull

Values <- c(SmallBull,MediumBull,LargeBull)    
Values[is.na(Values)] = 0 # NAs to 0
Categories <- c('Sm. Bull','Med. Bull','Lg. Bull') 
barplot(Values,names.arg = Categories,ylim=c(0,1),main="Bulls composition",ylab="Proportion of total bulls")
Values
...
Sql = paste("SELECT [Year], Cow, Calf, [Small bull], [Medium bull], [Large bull], Bull, Adult, Caribou, Unknown, [Total groups observed] as n
...
# Exclude raw data - provisional or certified only
Sql = paste("SELECT        Year, GroupSize_Mean AS [Mean group size], [Pct. calf], [Pct. cow], [Pct. bull (composition survey)] AS [Pct. bull], [Calves/100 cows], [Bulls/100 cows], [Total groups observed] AS n, [Pct. small bull], 
...
# Abandoned ggplot, too hard to make a graph
# ggplot(data=StatsDF,aes(x = Year)) +
#   geom_line(aes(y = `Bulls/100 cows`,colour="black"),size=1, linetype='solid') +
#   geom_line(aes(y = `Calves/100 cows`,colour="darkgray"), size=1, linetype='solid')  + 
#   xlab("Year") +
...
plot(StatsDF$Year, StatsDF$`Bulls/100 cows`, type = "l", xlab = "Year", ylab = "Ratio",ylim = c(0, 60))                      
lines(StatsDF$Year, StatsDF$`Calves/100 cows`, type = "l", col = "red")                   # Add second line
...
Figure `r FigureCount`. Historical sex and age ratios.

<!-- ```{r echo=FALSE} -->

<!-- # Plot the bull: cow ratio data -->

<!-- ggplot(data=StatsDF, aes(x=Year, y=`Bulls/100 cows`, group=1)) + -->
...
# ggplot(data=subset(StatsDF,`Pct. calf` != 100)) + # Subset to get rid of the spring surveys that have 100% calves
#   #geom_line(aes(x=Year,y = `Pct. calf`, colour = `Pct. calf`)) + # I don't know why colour becomes the legend label, but it does
#   #geom_line(aes(x=Year,y = `Pct. cow`, colour = `Pct. cow`)) + 
#   geom_ribbon(aes(x=Year,ymin=0,ymax=`Pct. calf`,colour="black"),fill="black") +
#   geom_ribbon(aes(x=Year,ymin=`Pct. calf`,ymax=`Pct. calf` + `Pct. bull`,colour="darkgray"),fill="darkgray") +
#   geom_ribbon(aes(x=Year,ymin=`Pct. bull`,ymax=`Pct. calf` + `Pct. bull`+`Pct. cow`,colour="lightgray"),fill="lightgray") +
#   labs(title = GraphTitle, color = "Categories\n")+
#   scale_color_manual(labels = c("Calves","Bulls","Cows"), values = c("black", "darkgray","lightgray"))  + 
#   theme_classic()
...
```{r echo=FALSE}
SightabilitySql = paste("SELECT [Total groups observed], [Marked groups], [Observed groups], [Marked groups that were observed], [Marked groups that were not observed], [Pct. marked groups observed], 
                         [Groups in the survey area], [Groups out of the survey area], [Marked groups in survey area], [Observed groups in survey area], [Marked groups observed in survey area], [Pct. marked groups observed in survey area], 
